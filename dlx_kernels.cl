...
void remove_column_d(int id, int *dlx, int dlx_size) {
...
void restore_column_d(int id, int *dlx, int dlx_size) {
  int *up, *down, *left, *right;
  UNLOAD(dlx, dlx_size);
...
kernel void exact_cover_kernel(global int *dlxs, global const int *dlx_props,
  ...
  int *up, *down, *left, *right;
  int *dlx = dlxs + g_id * dlx_size * 4;
  int *stack = stacks + l_id * N * N;
  UNLOAD(dlx, dlx_size);
